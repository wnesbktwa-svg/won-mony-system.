...
def home():
    ...
    <head>
        ...
        <title>Won Mony - Merchant Edition</title>
        <style>
            body { background: #000; color: #fff; font-family: sans-serif; text-align: center; padding: 15px; }
            .header { color: #00ff00; margin-bottom: 20px; font-weight: bold; }
            .box { background: #0a0a0a; border: 2px solid #00ff00; border-radius: 20px; padding: 20px; max-width: 400px; margin: 0 auto 20px; }
            input { width: 85%; padding: 12px; border-radius: 10px; border: 1px solid #333; background: #111; color: #fff; font-size: 22px; text-align: center; }
            ...
            .share-btn { 
                ...
                border-radius: 12px; width: 100%; font-weight: bold; font-size: 18px; 
                cursor: pointer; margin-top: 20px; max-width: 400px;
            }
            .footer { margin-top: 30px; font-size: 11px; color: #444; }
        </style>
        <script>
            let officialRate = 0;
            const blackMarketRate = 8.77; 
            ...
            function updateValues(usd) {
                ...
                // تحديث رابط الواتساب
                let msg = `*تقرير أسعار العملة (Won Mony V14)*%0A` +
                          `المبلغ: ${usd} دولار%0A` +
                          `------------------------%0A` +
                          `السعر الرسمي: ${offVal} د.ل%0A` +
                          `السوق الموازي: ${bmVal} د.ل%0A` +
                          `الفارق مالي: ${diff} د.ل`;
    ...
    <body>
        <h2 class="header">WON MONY - نسخة التجار</h2>
        ...
        <div class="footer">
            Won Mony Merchant System<br>
            تحديث مباشر: عين ليبيا & البورصة العالمية
        </div>
    </body>
    </html>
    ''')
